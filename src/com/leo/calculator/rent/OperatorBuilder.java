...
final class OperatorBuilder {
	...
	public static Operator fixedRate(BigDecimal fixed, BigDecimal rate) {
		return (s) -> new Process(fixed, rate, fixed.multiply(rate).divide(PERCENTAGE, DEFAULT_SCALE, RoundingMode.DOWN));
	}
	...
	public static Operator rate(BigDecimal rate) {
		return (s) -> new Process(s, rate, s.multiply(rate).divide(PERCENTAGE, DEFAULT_SCALE, RoundingMode.DOWN));
	}
	...
	public static Operator rangeRate(BigDecimal from, BigDecimal to, BigDecimal rate) {
		return (s) -> {
			...
			return new Process(from, s.min(to), target, rate, 
